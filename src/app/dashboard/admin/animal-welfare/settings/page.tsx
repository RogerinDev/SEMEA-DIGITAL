...
import { getAnimalWelfareSettings, updateAnimalWelfareSettings } from '@/app/actions/settings-actions';
import type { SectorSettings } from '@/types';
import { PawPrint, Loader2, PlusCircle, ShieldAlert, Trash2 } from 'lucide-react';
...
export default function AnimalWelfareSettingsPage() {
  ...
  async function onSubmit(data: z.infer<typeof settingsSchema>) {
    ...
    const result = await updateAnimalWelfareSettings(data);
    if (result.success) {
  ...
  if (loading) {
    return (
        <div>
            <PageTitle title="Gerenciar Conteúdo de Bem-Estar Animal" icon={PawPrint} />
  ...
  return (
    <>
      <PageTitle title="Gerenciar Conteúdo de Bem-Estar Animal" icon={PawPrint} description="Edite as informações que aparecem nas páginas públicas do setor."/>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          
          <Card>
            <CardHeader>
              <CardTitle>Pilares de Atuação (Projetos)</CardTitle>
              <CardDescription>Gerencie os pilares de atuação exibidos na página pública.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => appendProject({ id: crypto.randomUUID(), slug: '', title: '', description: '', active: true })}><PlusCircle className="mr-2 h-4 w-4" /> Adicionar Pilar/Projeto</Button>
            </CardContent>
          </Card>
